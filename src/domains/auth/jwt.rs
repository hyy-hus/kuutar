...
/// Encodes a JWT token with user_id, group_id, and an expiration TTL in seconds.
pub fn encode_jwt(
    user_id: Uuid,
    group_id: Uuid,
    secret: &str,
...
mod tests {
    ...
    fn test_encode_and_decode_jwt_success() {
        ...
        let token =
            encode_jwt(user_id, group_id, TEST_SECRET, 900).expect("Encoding JWT should succeed");
