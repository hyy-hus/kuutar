            ...
            <div className="flex-1 p-2 sm:p-4 max-w-6xl mx-auto w-full space-y-6 pb-12">
                ...
                {isLoading ? (
                    ...
                    <div className="p-6 border-2 border-rose-300 dark:border-rose-900 bg-rose-50 dark:bg-rose-950/40 rounded-md text-xs font-mono text-rose-800 dark:text-rose-300 flex items-center gap-2">
                        <AlertCircle size={16} />
                        <span>Tilastojen lataaminen epäonnistui. Yritä myöhemmin uudelleen.</span>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="p-4 border-2 border-stone-800 dark:border-stone-700 rounded-md bg-stone-100 dark:bg-stone-900 space-y-1">
                                <div className="flex items-center justify-between text-stone-500">
                                    <span className="text-xs font-mono font-bold uppercase">Varaukset</span>
                                    <Bookmark size={16} />
                                </div>
                            ...
                            <div className="p-4 border-2 border-stone-800 dark:border-stone-700 rounded-md bg-stone-100 dark:bg-stone-900 space-y-1">
                                <div className="flex items-center justify-between text-stone-500">
                                    <span className="text-xs font-mono font-bold uppercase">Resurssit</span>
                                    <Box size={16} />
                                </div>
                                <p className="text-2xl font-black text-stone-900 dark:text-stone-100 font-mono">
                            ...
                            <div className="p-4 border-2 border-stone-800 dark:border-stone-700 rounded-md bg-stone-100 dark:bg-stone-900 space-y-1">
                                <div className="flex items-center justify-between text-amber-600 dark:text-amber-400">
                                    <span className="text-xs font-mono font-bold uppercase">Odottavat</span>
                                    <Clock size={16} />
                                </div>
                                <p className="text-2xl font-black text-stone-900 dark:text-stone-100 font-mono">
                            ...
                            <div className="p-4 border-2 border-stone-800 dark:border-stone-700 rounded-md bg-stone-100 dark:bg-stone-900 space-y-1">
                                <div className="flex items-center justify-between text-stone-500">
                                    <span className="text-xs font-mono font-bold uppercase">Käyttäjät</span>
                                    <Users size={16} />
                                </div>
                                <p className="text-2xl font-black text-stone-900 dark:text-stone-100 font-mono">
                                    {stats?.total_users ?? 0}
                                ...
                                {stats?.top_resources && stats.top_resources.length > 0 ? (
                                    <ul className="space-y-3">
                                        {stats.top_resources.map((item) => {
                                            const maxCount = stats.top_resources[0]?.reservation_count || 1
                                            const pct = Math.round((item.reservation_count / maxCount) * 100)
                                                ...
                                                <li key={item.resource_id} className="space-y-1">
                                                    <div className="flex items-center justify-between text-xs font-mono">
                                                        ...
                                                        <span className="text-stone-500 shrink-0">
                                                            {item.reservation_count} varaus{item.reservation_count !== 1 && 'ta'}
                                                        </span>
                                                    </div>
                                    ...
                                    <p className="text-xs text-stone-500 font-mono">Ei vielä varaustilastoja saatavilla.</p>
                                )}
                            </div>
